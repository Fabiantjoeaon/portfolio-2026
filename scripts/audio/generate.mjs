...
function sceneDefaults() {
  return {
    meadow: {
      ...
      density: { sparseGrid: "8n", rateLow: 3, rateHigh: 10, ornamentEvery: 4 },
    ...
    cube: {
      ...
      density: { sparseGrid: "8n", rateLow: 2, rateHigh: 8, ornamentEvery: 5 },
    ...
    ice: {
      ...
      floor: {
        ...
        synth: {
          type: "synth",
          oscillator: "sine",
          envelope: { attack: 0.01, decay: 1.1, sustain: 0, release: 1.2 },
        },
      },
      wall: {
        pattern: "rippleHigh",
...
export function buildMusic({ analysis, brief = "", overrides = {}, log = console }) {
  ...
  const music = {
    ...
    quantize: { grid: "16n", collision: "push", maxPushSlots: 2 },
    ...
    pad: {
      ...
      reverbSend: 0.85,
      velocity: 0.6,
      oscillator: { type: "fatsawtooth", count: 2, spread: Math.round(12 + 30 * sideToMid) },
      envelope: {
        attack: 3,
        decay: 3,
        sustain: 0.75,
      ...
      lfo: {
        rate: 0.035,
        min: Math.round(clamp(brightness[0] * 1.1, 250, padCutoff * 0.8)),
        max: Math.round(padCutoff * 1.4),
      },
      detuneLfo: { rate: 0.07, depth: 7 },
    },
    patterns: PATTERNS,
