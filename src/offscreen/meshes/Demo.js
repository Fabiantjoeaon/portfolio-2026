...
import { SPH, GridPristine, MeshTransmissionNodeMaterial } from '@three-blocks/core';
...
export class Demo extends component( THREE.Object3D, {
	...
	init() {
		...
		materialSPH.colorNode = this.sph.buffers.directions.element( instanceIndex ).xyz.length().smoothstep( 4, 14 ).mix(
			materialColor,
			this.sph.buffers.directions.element( instanceIndex ).xyz.length().smoothstep( 7, 14 ).mul( 1 ).mix(
				color( 0x0000ff ), color( 0x0044ff ).mul( 1.5 )
			)
		);
	...
	onRaf( { gl, camera, pointer, delta } ) {
		...
		this.sph.ubos.rayDirection.value.copy( this.raycaster.ray.direction ).normalize();
